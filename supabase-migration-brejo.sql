-- ============================================================================
-- supabase-migration-brejo.sql
-- Adiciona a Pesquisa Eleitoral Brejo (MA) 2026 a um banco já em produção,
-- SEM apagar nenhuma tabela ou dado existente — ao contrário de
-- supabase-schema.sql, que faz reset completo.
--
-- O modelo é EAV (pesquisas -> entrevistas -> respostas), então Brejo não
...
-- para o município "Brejo".
-- ============================================================================

insert into public.pesquisas (nome, municipio, ativa)
select 'Pesquisa Eleitoral Brejo (MA) 2026', 'Brejo', true
where not exists (
  select 1 from public.pesquisas where municipio = 'Brejo'
);
