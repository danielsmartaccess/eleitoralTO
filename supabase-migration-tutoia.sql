-- ============================================================================
-- supabase-migration-tutoia.sql
-- Adiciona a Pesquisa Eleitoral Tutóia (MA) 2026 a um banco já em produção,
-- SEM apagar nenhuma tabela ou dado existente — ao contrário de
-- supabase-schema.sql, que faz reset completo.
--
-- O modelo é EAV (pesquisas -> entrevistas -> respostas), então Tutóia não
...
-- para o município "Tutóia".
-- ============================================================================

insert into public.pesquisas (nome, municipio, ativa)
select 'Pesquisa Eleitoral Tutóia (MA) 2026', 'Tutóia', true
where not exists (
  select 1 from public.pesquisas where municipio = 'Tutóia'
);
