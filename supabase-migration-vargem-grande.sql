-- ============================================================================
-- supabase-migration-vargem-grande.sql
-- Adiciona a Pesquisa Eleitoral Vargem Grande (MA) 2026 a um banco já em
-- produção, SEM apagar nenhuma tabela ou dado existente — ao contrário de
-- supabase-schema.sql, que faz reset completo.
--
-- O modelo é EAV (pesquisas -> entrevistas -> respostas), então Vargem Grande
...
-- para o município "Vargem Grande".
-- ============================================================================

insert into public.pesquisas (nome, municipio, ativa)
select 'Pesquisa Eleitoral Vargem Grande (MA) 2026', 'Vargem Grande', true
where not exists (
  select 1 from public.pesquisas where municipio = 'Vargem Grande'
);
