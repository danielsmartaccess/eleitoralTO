-- ============================================================================
-- supabase-migration-chapadinha.sql
-- Adiciona a Pesquisa Eleitoral Chapadinha (MA) 2026 a um banco já em
-- produção, SEM apagar nenhuma tabela ou dado existente — ao contrário de
-- supabase-schema.sql, que faz reset completo.
--
-- O modelo é EAV (pesquisas -> entrevistas -> respostas), então Chapadinha não
...
-- para o município "Chapadinha".
-- ============================================================================

insert into public.pesquisas (nome, municipio, ativa)
select 'Pesquisa Eleitoral Chapadinha (MA) 2026', 'Chapadinha', true
where not exists (
  select 1 from public.pesquisas where municipio = 'Chapadinha'
);
