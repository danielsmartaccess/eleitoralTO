-- ============================================================================
-- supabase-migration-sao-bernardo.sql
-- Adiciona a Pesquisa Eleitoral São Bernardo (MA) 2026 a um banco já em
-- produção, SEM apagar nenhuma tabela ou dado existente — ao contrário de
-- supabase-schema.sql, que faz reset completo.
--
-- O modelo é EAV (pesquisas -> entrevistas -> respostas), então São Bernardo
...
-- para o município "São Bernardo".
-- ============================================================================

insert into public.pesquisas (nome, municipio, ativa)
select 'Pesquisa Eleitoral São Bernardo (MA) 2026', 'São Bernardo', true
where not exists (
  select 1 from public.pesquisas where municipio = 'São Bernardo'
);
